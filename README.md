...
`(x,y)(y,z)`<br/><br/>Status line:<br/>Number of rule-based matches. | Hypersurfaces matching the given rule-based pattern. Using a prefix '-' or '^' matched hypersurfaces are excluded from the results. NOTE: Matching is done in `SPACE` mode and only projected to `TIME` mode by highlighting worldlines of the matched hypersurface. | `(1,2,3)(3,4,5)`<br/>`(x,y)(x,z,y)(x,u,y)`
